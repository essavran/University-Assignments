...
template <class T>
HexAdapter<T>::HexAdapter(int inputSize) : AbstractHex(inputSize) {
	setSize(boardSize);	//created board
}

template <class T>
HexAdapter<T>::HexAdapter(int inputSize, int inputGameMode) : AbstractHex(inputSize, inputGameMode){
	setSize(boardSize);	//created board
}
...
template <class T>
void HexAdapter<T>::setSize(int board){
	boardSize = board;	
...
template <class T>
void HexAdapter<T>::play(Cell cellS){
...
//these are from previous homeworks
template <class T>
bool HexAdapter<T>::win_o(void){
	int counter[4];
	counter[0] = boardSize;
	counter[3] = -1;
	//counter[1]; row
...
template <class T>
void HexAdapter<T>::winRecursion_o(int counter[4]){
	int row = counter[1],
		column = counter[2];
...
template <class T>
void HexAdapter<T>::winRecursion_x(int counter[4]){
	int row = counter[1],
		column = counter[2];
...
template <class T>
void HexAdapter<T>::convertLowerCase(void){
	for(int i=0; i<boardSize; i++){
		for(int j=0; j<boardSize; j++){
