...
int main(){
	...
	if( choice[0] == '1' ){ // a single game
		...
		for(int i=0; i<active; i++){
			...
			activeGames.push_back( Hex(true) );
		}
		do{
			cout << endl << "----> Let's play the Game" << currentGame+1 << endl;
			activeGames[currentGame].setStart(1);
			...
			cout << finishedGameCounter << " " << active << endl;
		} while( finishedGameCounter != active );

		number = Hex().numOfMarkedCells();
		...
		for(int i=0; i<active; i++){ //Compares Hexs
			...
			for(int j=0; j<active; j++){
				if( i != j ){
					...
					if( activeGames[i] == activeGames[j] ){
