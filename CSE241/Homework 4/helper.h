...
public:
	...
	friend ostream& operator<<(ostream& outputStream, Cell **hexCells);
	friend istream& operator>>(istream& inputStream, Hex& hexGame);
	friend bool operator==(Hex hexGame1, Hex hexGame2);

private:
	void createMoves();//create the moves array
	Hex& operator--(int ignore); //postfix
	Hex& operator--(); //prefix
	int ** moves; //to hold the previous moves
	void addMoves(int row, int column); //CELL gönder
	int numOfMoves = 0;
	bool isPreUndo(const string &input);//Check the input is PREUNDO
	bool isPostUndo(const string &input);//Check the input is POSTUNDO
	int score(cellState pState);//calculate scores
	bool isScore(const string &input);//Check the input is SCORE
	void printLastScore();//when the game is over, prints last scores

	//This function shows the user the size options. 
	//It gets the size from user.
	//If the input is valid, function returns the size.
	inline void setBoardSize();
	...
	Cell ** hexCells;
	int ** checkArray; //To control of the cell positions
};
#endif